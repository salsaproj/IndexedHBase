 ...
 * words as row keys, column name as document IDs, and cell values as words' positions in corresponding
 ...
 * <p/>
 * Usage: java iu.pti.hbaseapp.GeneralPosIndexFileBuilder [text table name] [text table column family] [text table qualifier]
 * [index table name] [index column family] [HDFS output path] [intermediate compression type]
 * 
 * @author gaoxm
 */
public class GeneralPosIndexFileBuilder {
	/**
	 * Mapper class used by {@link #GeneralPosIndexFileBuilder}.
	 */
	public static class GpifbMapper extends TableMapper<KeyValueComparable, NullWritable> {
		...
		protected void map(ImmutableBytesWritable rowKey, Result result, Context context) throws IOException, InterruptedException {
			...
			HashMap<String, ByteArrayOutputStream> termPosMap = GeneralHelpers.getTermPosesByLuceneAnalyzer(Constants.getLuceneAnalyzer(),
					content, Constants.DUMMY_FIELD_NAME);
			for (Map.Entry<String, ByteArrayOutputStream> e : termPosMap.entrySet()) {
				String term = e.getKey();
				KeyValueComparable kv = new KeyValueComparable(Bytes.toBytes(term), indexTableCfBytes, rowKey.get(), e.getValue().toByteArray());
	...
	/**
	 * Reducer class used by {@link #GeneralPosIndexFileBuilder}.
	 */
	public static class GpifbReducer extends Reducer<KeyValueComparable, NullWritable, ImmutableBytesWritable, KeyValue> {
		@Override
	...
	public static Job configureJob(Configuration conf, String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf,	"Building pos_index from " + textTableName);
		conf = job.getConfiguration();
		job.setJarByClass(GpifbMapper.class);
		TableMapReduceUtil.initTableMapperJob(textTableName, scan, GpifbMapper.class, KeyValueComparable.class, 
												NullWritable.class, job, true);
		job.setReducerClass(GpifbReducer.class);
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length < 7) {
			...
			System.err.println("Usage: GeneralFreqIndexFileBuilder <text table name> <text table column family> <text table qualifier>"
								+ " <index table name> <index column family> <HDFS output path> <intermideiate compression type>");
			System.exit(-1);
		}
